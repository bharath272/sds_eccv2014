...
void* MaskedWindowDataLayerPrefetch(void* layer_pointer) {
  MaskedWindowDataLayer<Dtype>* layer = 
      reinterpret_cast<MaskedWindowDataLayer<Dtype>*>(layer_pointer);
...
//  CHECK_EQ(mean_off, 14);
  ...
  for (int img_num =0; img_num<num_images; ++img_num) {
    ...
    for (int is_fg = 0; is_fg < 2; ++is_fg) {
      for (int dummy = 0; dummy < num_samples[is_fg]; ++dummy) {
        ...
        int x1 = window[MaskedWindowDataLayer<Dtype>::X1];
        int y1 = window[MaskedWindowDataLayer<Dtype>::Y1];
        int x2 = window[MaskedWindowDataLayer<Dtype>::X2];
        int y2 = window[MaskedWindowDataLayer<Dtype>::Y2];
	    int index_in_image = window[MaskedWindowDataLayer<Dtype>::INDEX_IN_IMAGE];  
        int pad_w = 0;
        int pad_h = 0;
  ...
  //      CHECK_LT(y2, cv_img.rows);
        ...
        // get window label
        top_label[itemid] = window[MaskedWindowDataLayer<Dtype>::LABEL];
...
template <typename Dtype>
MaskedWindowDataLayer<Dtype>::~MaskedWindowDataLayer<Dtype>() {
  JoinPrefetchThread();
}


template <typename Dtype>
void MaskedWindowDataLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  while (infile >> hashtag >> image_index) {
    ...
    for (int i = 0; i < num_windows; ++i) {
      ...
      vector<float> window(MaskedWindowDataLayer::NUM);
      window[MaskedWindowDataLayer::IMAGE_INDEX] = image_index;
      window[MaskedWindowDataLayer::LABEL] = label;
      window[MaskedWindowDataLayer::OVERLAP] = overlap;
      window[MaskedWindowDataLayer::X1] = x1;
      window[MaskedWindowDataLayer::Y1] = y1;
      window[MaskedWindowDataLayer::X2] = x2;
      window[MaskedWindowDataLayer::Y2] = y2;
      window[MaskedWindowDataLayer::INDEX_IN_IMAGE] = index_in_image;
      
      // add window to foreground list or background list
      if (overlap >= this->layer_param_.window_data_param().fg_threshold()) {
        CHECK_GT(window[MaskedWindowDataLayer::LABEL], 0);
        ...
        window[MaskedWindowDataLayer::LABEL] = 0;
        window[MaskedWindowDataLayer::OVERLAP] = 0;
        imgwin.bg_windows_index.push_back(windows_.size());
      }
      windows_.push_back(window);
      label_hist[window[MaskedWindowDataLayer::LABEL]]++;
    }
  ...
  LOG(INFO) << "output data size: " << (*top)[0]->num() << ","
      ...
      << (*top)[0]->width();
  // label
  (*top)[1]->Reshape(this->layer_param_.window_data_param().batch_size(), 1, 1, 1);
  ...
  CHECK(!pthread_create(&thread_, NULL, MaskedWindowDataLayerPrefetch<Dtype>,
      reinterpret_cast<void*>(this))) << "Pthread execution failed.";
  DLOG(INFO) << "Prefetch initialized.";
}

template <typename Dtype>
void MaskedWindowDataLayer<Dtype>::CreatePrefetchThread() {
...
template <typename Dtype>
void MaskedWindowDataLayer<Dtype>::JoinPrefetchThread() {
  CHECK(!pthread_join(thread_, NULL)) << "Pthread joining failed.";
}

template <typename Dtype>
unsigned int MaskedWindowDataLayer<Dtype>::PrefetchRand() {
  CHECK(prefetch_rng_);
...
template <typename Dtype>
Dtype MaskedWindowDataLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top) {
...
INSTANTIATE_CLASS(MaskedWindowDataLayer);
