...
if(exist(stat_file, 'file'))
    ...
    for i=1:num_images
        ...
        %load random image
        feats=load_feats(feat_dir, imnames{ri(i)});
...
for j=1:numel(to_train)
    ...
    if(isempty(curr_keys{j})) curr_keys{j}=zeros(0,2); end
end
...
y1=load(fullfile(featdir,'ground_truth_sp_approx_LBP_f_pca_2500_noncent', [image_id '.mat']));
y2=load(fullfile(featdir,'ground_truth_sp_approx_SIFT_GRAY_f_g_pca_5000_noncent', [image_id '.mat']));
y3=load(fullfile(featdir,'ground_truth_sp_approx_SIFT_GRAY_mask_pca_5000_noncent', [image_id '.mat']));
 feats=[[y1.D;y2.D;y3.D] [x1.D;x2.D; x3.D]]';
